package com.hereliesaz.blusnu.ui.blesa
...
import com.hereliesaz.blusnu.data.BlesaModule
...
/**
 * ViewModel for the BLESA (BLE Spoofing Attack on Reconnections) screen.
 *
 * Bridges the UI and the [BlesaModule]. Manages the attack lifecycle
 * including cancellation and resource cleanup.
 */
class BlesaViewModel(
    application: Application,
    private val deviceRepository: DeviceRepository
) : AndroidViewModel(application) {

    private val blesaModule = BlesaModule(application)
    ...
    init {
        viewModelScope.launch {
            deviceRepository.allDevices.collect { allDevices ->
                // BLESA targets BLE reconnections. Filter out Classic-only devices.
    ...
    /**
     * Executes the BLESA reconnection spoofing test.
     */
    fun startAttack() {
        ...
        val taskId = "blesa_${System.currentTimeMillis()}"
        currentTaskId = taskId
        ActionLogger.log("BLESA Reconnection Test: Starting on $displayName")
        ActiveTaskManager.add(taskId, "BLESA Reconnection Test", "Targeting $displayName")

        attackJob = viewModelScope.launch {
            try {
                blesaModule.startAttack(device, getApplication()).collect { log ->
                    _logs.value = _logs.value + log
                }
                val resultSummary = _logs.value.joinToString("\n").take(200)
                ActionLogger.log("BLESA Reconnection Test: Finished on $displayName")
