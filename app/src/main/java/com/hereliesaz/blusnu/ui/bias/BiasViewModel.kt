package com.hereliesaz.blusnu.ui.bias
...
import com.hereliesaz.blusnu.data.BiasModule
...
/**
 * ViewModel for the BIAS attack screen.
 *
 * Bridges the UI and the [BiasModule]. Manages the attack lifecycle
...
class BiasViewModel(
    ...
    private val biasModule = BiasModule()
    ...
    init {
        viewModelScope.launch {
            deviceRepository.allDevices.collect { allDevices ->
                // BIAS targets BR/EDR (Classic) authentication. Filter out BLE-only devices.
    ...
    fun startAttack() {
        ...
        val taskId = "bias_${System.currentTimeMillis()}"
        currentTaskId = taskId
        ActionLogger.log("BIAS Attack: Starting on $displayName")
        ActiveTaskManager.add(taskId, "BIAS Attack", "Targeting $displayName")

        attackJob = viewModelScope.launch {
            try {
                biasModule.startAttack(device).collect { log ->
                    _logs.value = _logs.value + log
                }
                val resultSummary = _logs.value.joinToString("\n").take(200)
                ActionLogger.log("BIAS Attack: Finished on $displayName")
