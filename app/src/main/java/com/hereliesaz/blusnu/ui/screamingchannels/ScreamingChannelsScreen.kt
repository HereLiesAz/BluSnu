...
/**
 * Screen for the Screaming Channels (Side-Channel) attack.
 *
 * Captures electromagnetic leakage from BLE chips via SDR hardware.
 * Analyzes radio emissions for crypto operation signatures to attempt
 * AES key recovery.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ScreamingChannelsScreen(viewModel: ScreamingChannelsViewModel) {
        ...
        Column(
            ...
            Text(
                text = "Captures electromagnetic leakage from BLE chips via SDR hardware. " +
                        "Analyzes radio emissions for crypto operation signatures to " +
                        "attempt AES key recovery.",
            ...
            ExposedDropdownMenuBox(
                ...
                TextField(
                    value = when (selectedMode) {
                        ScreamingMode.CAPTURE_TRACES -> "Capture Traces"
                        ScreamingMode.ANALYZE_LEAKAGE -> "Analyze Leakage"
                        ScreamingMode.KEY_RECOVERY -> "Key Recovery"
                    },
                    onValueChange = {},
                    readOnly = true,
                    label = { Text("Capture Mode") },
                ...
                ExposedDropdownMenu(
                    ...
                    DropdownMenuItem(
                        text = { Text("Capture Traces") },
                        onClick = {
                            viewModel.selectMode(ScreamingMode.CAPTURE_TRACES)
                            modeExpanded = false
                        }
                    )
                    DropdownMenuItem(
                        text = { Text("Analyze Leakage") },
                        onClick = {
                            viewModel.selectMode(ScreamingMode.ANALYZE_LEAKAGE)
                            modeExpanded = false
                        }
                    )
                    DropdownMenuItem(
                        text = { Text("Key Recovery") },
                        onClick = {
                            viewModel.selectMode(ScreamingMode.KEY_RECOVERY)
                            modeExpanded = false
                        }
                    )
                }
            ...
            if (logs.isNotEmpty()) {
                ResultActions(
                    ...
                    label = "Screaming Channels Results",
                    modifier = Modifier.fillMaxWidth()
                )
            }
