...
fun TargetManagementScreen(
    ...
    var selectedDeviceForNotes by remember { mutableStateOf<TargetDevice?>(null) }
    ...
    Column(modifier = modifier.padding(16.dp)) {
            ...
            ExposedDropdownMenu(expanded = filterExpanded, onDismissRequest = { filterExpanded = false }) {
                when(filterType) {
                    ...
                    FilterType.Favorites -> {
                        DropdownMenuItem(text = { Text("Show Favorites") }, onClick = {
                            viewModel.addFilter(FilterType.Favorites, true)
                            filterExpanded = false
                        })
                        DropdownMenuItem(text = { Text("Show All") }, onClick = {
                            viewModel.removeFilter(FilterType.Favorites)
                            filterExpanded = false
                        })
                    }
                }
        ...
        if (selectedDeviceForNotes != null) {
            EditNotesDialog(
                device = selectedDeviceForNotes!!,
                onDismiss = { selectedDeviceForNotes = null },
                onSave = { notes ->
                    viewModel.saveNotes(selectedDeviceForNotes!!.macAddress, notes)
                    selectedDeviceForNotes = null
                }
            )
        }
            ...
            LazyColumn {
                items(state.discoveredDevices, key = { it.macAddress }) { device ->
                    DeviceListItem(
                        ...
                        viewModel = viewModel,
                        navController = navController,
                        onLongPress = { selectedDeviceForNotes = device },
                        isDiscoveringServices = state.isDiscoveringServices.contains(device.macAddress),
                        isCheckingVulnerabilities = state.isCheckingVulnerabilities.contains(device.macAddress)
                    )
                }
            }
        }
...
fun DeviceListItem(
    ...
    navController: NavController,
    onLongPress: () -> Unit,
    isDiscoveringServices: Boolean,
    isCheckingVulnerabilities: Boolean
) {
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(12.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text(text = device.name ?: "Unknown", fontWeight = FontWeight.Bold, modifier = Modifier.weight(1f))
                IconButton(onClick = { viewModel.toggleFavorite(device.macAddress) }) {
                    Icon(
                        imageVector = if (device.isFavorite) Icons.Filled.Star else Icons.Outlined.StarBorder,
                        contentDescription = "Favorite",
                        tint = if (device.isFavorite) Color.Yellow else Color.Gray
                    )
                }
            }
            if (device.notes.isNotBlank()) {
                Text(text = device.notes)
            }
            Row {
...
fun EditNotesDialog(
    ...
    AlertDialog(
        ...
        title = { Text("Edit Notes for ${device.name ?: device.macAddress}") },
        text = {
            TextField(
                value = notes,
