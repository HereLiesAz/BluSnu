package com.hereliesaz.blusnu.ui.blur
...
import com.hereliesaz.blusnu.data.BlurModule
...
/**
 * ViewModel for BLUR Attack (CVE-2020-15802).
 *
 * Bridges the UI and the [BlurModule]. Manages the attack lifecycle
 * including cancellation and resource cleanup. Filters devices to
 * DUAL protocol only, as BLUR requires dual-mode (BR/EDR + BLE) targets
 * for cross-transport key derivation testing.
 */
class BlurViewModel(
    ...
    private val blurModule = BlurModule()
    ...
    init {
        viewModelScope.launch {
            deviceRepository.allDevices.collect { allDevices ->
                // BLUR targets CTKD between BR/EDR and BLE. Filter to DUAL only.
    ...
    fun startAttack() {
        ...
        val taskId = "blur_${System.currentTimeMillis()}"
        currentTaskId = taskId
        ActionLogger.log("BLUR Attack: Starting on $displayName")
        ActiveTaskManager.add(taskId, "BLUR Attack", "Targeting $displayName")

        attackJob = viewModelScope.launch {
            try {
                blurModule.startAttack(device).collect { log ->
                    _logs.value = _logs.value + log
                }
                val resultSummary = _logs.value.joinToString("\n").take(200)
                ActionLogger.log("BLUR Attack: Finished on $displayName")
