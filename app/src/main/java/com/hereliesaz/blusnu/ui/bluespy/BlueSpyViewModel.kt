package com.hereliesaz.blusnu.ui.bluespy
...
import com.hereliesaz.blusnu.data.BlueSpyModule
...
/**
 * ViewModel for BlueSpy Audio Interception.
 *
 * Bridges the UI and the [BlueSpyModule]. Manages the attack lifecycle
 * including recording state, cancellation, and resource cleanup.
 */
class BlueSpyViewModel(
    application: Application,
    private val deviceRepository: DeviceRepository
) : AndroidViewModel(application) {

    private val blueSpyModule = BlueSpyModule()
    ...
    init {
        viewModelScope.launch {
            deviceRepository.allDevices.collect { allDevices ->
                // BlueSpy targets BR/EDR audio devices. Filter to Classic and Dual-mode.
    ...
    /**
     * Executes the BlueSpy audio interception attack.
     */
    fun startAttack() {
        ...
        val taskId = "bluespy_${System.currentTimeMillis()}"
        currentTaskId = taskId
        ActionLogger.log("BlueSpy Audio Interception: Starting on $displayName")
        ActiveTaskManager.add(taskId, "BlueSpy Audio Interception", "Targeting $displayName")

        attackJob = viewModelScope.launch {
            try {
                blueSpyModule.startAttack(device).collect { log ->
                    _logs.value = _logs.value + log
                }
                val resultSummary = _logs.value.joinToString("\n").take(200)
                ActionLogger.log("BlueSpy Audio Interception: Finished on $displayName")
    ...
    /**
     * Stops the running attack and any active audio recording.
     */
    fun stopAttack() {
        blueSpyModule.stopRecording()
        attackJob?.cancel()
        attackJob = null
    }
