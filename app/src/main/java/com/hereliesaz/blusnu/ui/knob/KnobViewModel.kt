package com.hereliesaz.blusnu.ui.knob
...
import com.hereliesaz.blusnu.data.KnobModule
...
/**
 * ViewModel for the KNOB Attack screen.
 *
 * Bridges the UI and the [KnobModule]. Manages the attack lifecycle
 * including cancellation and resource cleanup. KNOB targets BR/EDR
 * (Classic) encryption key negotiation, so only Classic and Dual devices
...
class KnobViewModel(
    ...
    private val knobModule = KnobModule()
    ...
    init {
        viewModelScope.launch {
            deviceRepository.allDevices.collect { allDevices ->
                // KNOB targets BR/EDR (Classic) key negotiation. Filter out BLE-only devices.
    ...
    fun startAttack() {
        ...
        val taskId = "knob_${System.currentTimeMillis()}"
        currentTaskId = taskId
        ActionLogger.log("KNOB Attack: Starting on $displayName")
        ActiveTaskManager.add(taskId, "KNOB Attack", "Targeting $displayName")

        attackJob = viewModelScope.launch {
            try {
                knobModule.startAttack(device).collect { log ->
                    _logs.value = _logs.value + log
                }
                val resultSummary = _logs.value.joinToString("\n").take(200)
                ActionLogger.log("KNOB Attack: Finished on $displayName")
